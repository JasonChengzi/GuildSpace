...
#define SECTIONOFTABLEVIEW 1
#define ROWOFSECTION 1
#define DEFAULTINDENTIFIER @"cell"
#define HEIGHTOFROW 139.5f
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:DEFAULTINDENTIFIER];
    if(cell == nil){
//        cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:DEFAULTINDENTIFIER];
        ...
        btn_nickname.tag = 600000 + indexPath.row;
        [btn_nickname addTarget:self action:@selector(gotoUserDetail:) forControlEvents:UIControlEventTouchUpInside];
...
//            NSLog(@"%s || %i", __FUNCTION__, self.currentPage);
    ...
    if(self.array_messages.count < 20){
...
- (void)gotoUserDetail:(UIButton *)button{
    PersonalCardViewController *personalCardViewCtrl = [[PersonalCardViewController alloc]init];
    
    personalCardViewCtrl.username = [self.array_messages[(button.tag - 600000)]objectForKey:@"nickname"];
    personalCardViewCtrl.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:personalCardViewCtrl animated:YES];
}
