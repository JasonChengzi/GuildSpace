...
+ (void)sendMessageWithContent:(NSString *)content andTarget:(id)target andSelector:(SEL)selector{
    ...
    NSInteger user_id = [[Common sharedCommon]getCurrentUserId];
...
+ (void)getUserInfoWithUsername:(NSString *)username andTarget:(id)target andSelector:(SEL)selector{
    NSDictionary *dic = @{USER_NICKNAME: username};
    NSString *url = [Common getUrlCombineWithModule:APIUSER andFunction:GETUSERINFO];
    [Network postDataWithParams:dic andFrom:url andTarget:target andSelector:selector];
}
