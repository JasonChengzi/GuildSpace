...
- (void)setUserWithUsername:(NSString *)username andUser_id:(NSInteger)user_id andPhone:(NSString *)phone{
...
- (NSDictionary *)getCurrentUser{
    NSDictionary *dic_user = @{USER_ID: [Common getStringWithInteger:self.user_id], USER_USERNAME: self.username, USER_PHONE: self.phone};
    return dic_user;
}
...
+(CGRect)getVerticalRectlWithSuperView:(UIView *)superView andYOrigin:(float)y andSize:(CGSize)size{
