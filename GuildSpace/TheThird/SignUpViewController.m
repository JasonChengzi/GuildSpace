...
- (void)initView{
    ...
    [btn_start setFrame:[Common getVerticalRectlWithSuperView:self.view andYOrigin:self.txt_username.frame.origin.y + self.txt_username.frame.size.height + 10 andSize:[Common getContentSizeWithContent:btn_start.titleLabel.text andFont:btn_start.titleLabel.font]]];
...
//        [[Common sharedCommon]setUserWithUsername:[dic_user objectForKey:@"username"] andUser_id:[[dic_user objectForKey:@"id"] integerValue] andPhone:[dic_user objectForKey:@"phone"]];
        NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
        [userDefault setObject:[NSNumber numberWithBool:YES] forKey:@"firstOpenFlag"];
        [userDefault synchronize];
//        [dic_user writeToFile:[FileManager getFullDocumentPath:@"userData"] atomically:YES];
        NSDictionary *savingDic = [NSDictionary dictionaryWithObjectsAndKeys:[dic objectForKey:@"username"], @"username", [dic objectForKey:@"id"], @"id", [dic objectForKey:@"phone"], @"phone", nil];
//        NSLog(@"%s || 从文件读取的登陆账号信息:%@", __FUNCTION__, savingDic);
