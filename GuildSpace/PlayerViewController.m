...
//                [(PlayerTopView *)cell setupTopViewWithStatus:NO andHeight:HEIGHTOFSECTIONONE];
                ...
                switch (indexPath.row) {
                    case 0:
                        cell.textLabel.text = [NSString stringWithFormat:@"当前昵称：%@", [[Common sharedCommon]getCurrentUsername]];
                        cell.detailTextLabel.text = @"点击更改";
                        break;
                    case 1:{
                        NSString *string;
                        NSString *detailString;
                        if([[[Common sharedCommon]getCurrentUserPhone]isEqualToString:@""] || [[[Common sharedCommon]getCurrentUserPhone]isEqualToString:@"0"] || [[[Common sharedCommon]getCurrentUserPhone] length] < 11){
                            string = [NSString stringWithFormat:@"未绑定手机"];
                            detailString = [NSString stringWithFormat:@"立即绑定"];
                        }else{
                            string = [NSString stringWithFormat:@"%@", [[Common sharedCommon]getCurrentUserPhone]];
                            detailString = [NSString stringWithFormat:@"前往更改"];
                        }
